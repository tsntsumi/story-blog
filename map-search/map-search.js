...
const LOC = (process.env.LOC || "36.39032241074826, 140.44528953941983").split(
  /, */
)
const LAT = process.env.LAT || LOC[0]
const LNG = process.env.LNG || LOC[1]
...
const photos = "https://maps.googleapis.com/maps/api/place/photo?"
...
;(async () => {
  ...
  for await (p of places) {
    ...
    const detail_config = {
      method: "get",
      url: `${detail}?place_id=${placeid}&fields=name,rating,formatted_address,formatted_phone_number,business_status,website,user_ratings_total,reviews,photos,type&language=ja&key=${apikey}`,
      headers: {}
    }

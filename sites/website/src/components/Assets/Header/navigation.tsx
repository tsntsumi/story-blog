...
export default function Navigation({
  ...
  const SubmenuItems = ({ menu }: { menu: Menu[] }): React.ReactChild => {
    ...
    return (
      <>
        <div
          className={`absolute top-8 z-30 w-[250px] flex flex-col py-4 bg-gold-dark text-gold rounded-md ${transClass}`}
        >
          {menu.map((m) => {
  ...
  return (
    <div
      className={`flex flex-nowrap w-full gap-2 z-50 items-center justify-around m-0 p-0 text-xs xs:text-[7px]`}
    >
      {menu.map((m) => (
